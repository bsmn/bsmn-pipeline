...
DONE=$SM/run_status/VAF_filters.calculation.malign.ploidy_$PL.done
...
if [[ -f $DONE ]]; then
    ...
    $PYTHON3 $PIPE_HOME/utils/somatic_vaf.2.py -q 20 -Q 20 -b "$BAMS" -r $REFVER -c $CONDA_ENV -n $((NSLOTS-2)) $IN > $VAF
