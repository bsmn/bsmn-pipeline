...
DONE1=$SM/run_status/Mayo_filters.strand.malign.ploidy_$PL.done
if [[ -f $DONE1 ]]; then
    echo "Skip calculating strand bias. Already done."
else
    cut -f1-4 $IN | $PYTHON3 $PIPE_HOME/utils/strand_bias.2.py -q 20 -Q 20 -b "$BAMS" -n $((NSLOTS-2)) > $STR
    mkdir -p $SM/run_status
    touch $DONE1
fi
printf -- "---\n[$(date)] Finish generate strand bias info.\n"

printf -- "\n[$(date)] Start generate repeat info.\n---\n"
DONE2=$SM/run_status/Mayo_filters.repeat.malign.ploidy_$PL.done
if [[ -f $DONE2 ]]; then
...
printf -- "---\n[$(date)] Finish generate repeat info.\n"
